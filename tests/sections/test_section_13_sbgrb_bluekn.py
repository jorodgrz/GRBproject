...
def test_logz_max_physical_below_grid_max(_section_13_cell_source: str):
    ...
    m = re.search(r"^LOGZ_MAX_PHYSICAL\s*=\s*([+-]?\d+(?:\.\d+)?)\s*$", src, re.MULTILINE)
    ...
    # The truncation must actually drop bins, otherwise it is a no-op.
    # Pre-fix code used ``edges_logZ[-1]`` ~ log(grid_max) + 0.05, so
    # any value <= grid_max is meaningful; the fiducial sits at -1.6
    # and drift > 0.5 dex is suspicious.
    assert -2.5 < logz_max < -1.5, (
        f"LOGZ_MAX_PHYSICAL = {logz_max:.3f} sits outside the (-2.5, "
        f"-1.5) window; the displayed axis would either keep too much "
        f"of the under-sampled tail or hide too much of the high-Z "
        f"f(Z) peak."
    )


def test_section_13_top_panel_legend_outside_axes(_section_13_cell_source: str):
...
def test_weighted_poisson_density_sigma_formula():
    ...
    expected_var = np.array([0.5**2 + 1.0**2 + 2.0**2, 0.3**2 + 0.7**2])
...
def _build_appendix_setup(bns_a_path):
    ...
    from grb_io import METALLICITY_GRID, load_bns_with_channels, read_expected_local_rate
    ...
    _, _, p_draw = fci.find_metallicity_distribution(
        redshifts, min_logZ_COMPAS=np.log(Z.min()), max_logZ_COMPAS=np.log(Z.max())
    )
...
def test_sbgrb_bluekn_integrand_at_z_merge_4_5_lands_on_dip(bns_a_path):
    ...
    contrib = wi > 0
    log10_Z_mean = float(np.average(log10_Z_sb[contrib], weights=wi[contrib]))
