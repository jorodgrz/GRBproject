...
Council 2026-05-06 Reviewer 4 flagged ``compas_python_utils`` (pinned in
``environment.yml`` to commit ``81722d4...``) as a single point of
failure: a STROOPWAFEL weight-extraction bug in that package would make
every rate in the manuscript systematically wrong.  This file ships the
escalating defenses we actually have:
...
3. ``test_find_metallicity_distribution_recovers_known_distribution``:
   ...
   entries, (c) per-redshift integration sums to unity (the upstream's
   own normalisation invariant), (d) the location of the metallicity
   peak shifts with z per the Neijssel et al. (2019) Eq. 7
   ``mu(z) = ln(mu_0 * 10**(muz * z)) - 0.5 * sigma(z)**2`` law.  Any
   silent change to the upstream STROOPWAFEL extraction or the
   Neijssel parameters would fail (c) or (d).
"""
...
def test_compas_cosmic_integration_module_imports():
    ...
    assert hasattr(ci, "find_metallicity_distribution") or hasattr(
        ci, "find_sfr"
...
def test_find_metallicity_distribution_recovers_known_distribution(bns_a_path):
    ...
    Closes Council I3 (Reviewer 4 single-point-of-failure) by anchoring
    ``compas_python_utils`` ``find_metallicity_distribution`` to four
    invariants that any future commit bump must continue to satisfy:
    ...
    dPdlogZ, metals, p_draw = ci.find_metallicity_distribution(
        z_grid, min_logZ_COMPAS, max_logZ_COMPAS, step_logZ=step_logZ,
    )
    ...
    assert dPdlogZ.shape[1] == metals.size, (
        f"dPdlogZ second axis {dPdlogZ.shape[1]} != metals length "
        f"{metals.size}"
    )
    ...
    np.testing.assert_allclose(
        row_integrals, np.ones_like(row_integrals), rtol=1e-6,
        err_msg=("upstream find_metallicity_distribution row integrals "
